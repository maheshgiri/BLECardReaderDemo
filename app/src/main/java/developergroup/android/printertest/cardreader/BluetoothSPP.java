...
public class BluetoothSPP {
    ...
    private final Handler mHandler = new Handler() {
        public void handleMessage(Message msg) {
            ...
            case BluetoothState.MESSAGE_WRITE:
                Log.d("DEBUG", "handleMessage: Writting message"+msg.obj.toString());

                break;
            case BluetoothState.MESSAGE_READ:
                Log.d("DEBUG", "handleMessage: Reading Messages");
    ...
    public void connect(String address) {
        BluetoothDevice device = mBluetoothAdapter.getRemoteDevice(address);
        mChatService.connect(device);
        Log.d("DEBUG", "connect: Device conncecting"+device.getAddress());
    }
    ...
    public void send(byte[] data, boolean CRLF) {
        Log.d("DEBUG", "send: Data Sending"+data.length);
        if(mChatService.getState() == BluetoothState.STATE_CONNECTED) {
            Log.d("DEBUG", "send: STATE CONNECTED");
            if(CRLF) {
            ...
            Log.d("DEBUG", "send: Data written");
        }
    }
    
    public String send(String data, boolean CRLF) {
        if(mChatService.getState() == BluetoothState.STATE_CONNECTED) {
            if(CRLF) 
                data += "\r\n"; 
            return mChatService.write(data.getBytes());
        }
        return null;
    }
